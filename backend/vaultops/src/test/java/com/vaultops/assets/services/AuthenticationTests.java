...
public class AuthenticationTests {
        ...
        @Test
        @DisplayName("Verify duplicate registration returns 400 for active user, but succeeds for pending user")
        void testDuplicateRegistrationReturns400() throws Exception {
                ...
                // First registration: user is PENDING, should succeed
                mockMvc.perform(post("/api/auth/register")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(request)))
                                .andExpect(status().isOk());

                // Second registration: user is still PENDING, should also succeed
                mockMvc.perform(post("/api/auth/register")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(request)))
                                .andExpect(status().isOk());
                ...
                verifyRequest.setEmail("duplicate@example.com");
                ...
                // Third registration: user is now ACTIVE, should fail with 400 Bad Request
                mockMvc.perform(post("/api/auth/register")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(request)))
        ...
        void testProtectedEndpointAccessibleWithJwt() throws Exception {
                String token = jwtTokenProvider.generateAccessToken("active@example.com", UserRole.USER.name(),
                                "Active User", 1L);
